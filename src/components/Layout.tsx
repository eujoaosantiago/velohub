...
export const Layout: React.FC<LayoutProps> = ({ children, currentPage, onNavigate, onAddVehicle, user, onLogout }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col md:flex-row transition-colors duration-300">
      ...
      {/* Sidebar Navigation */}
      <aside className={`
        fixed inset-0 z-40 bg-slate-900/95 backdrop-blur-xl transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:w-64 md:border-r md:border-slate-800 md:bg-slate-900 md:h-screen md:sticky md:top-0 flex flex-col
        ${isMobileMenuOpen ? 'translate-x-0 pt-20' : '-translate-x-full md:pt-0'}
      `}>
        <div className="hidden md:flex h-20 items-center px-6 border-b border-slate-800 flex-shrink-0">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent hover:scrollbar-thumb-slate-600 p-4 space-y-2 md:mt-0">
        ...
        <div className="border-t border-slate-800 pt-4 mt-auto space-y-3 p-4 flex-shrink-0 bg-slate-900/50">
           {/* Theme Toggle */}
           <button 
              onClick={toggleTheme}
