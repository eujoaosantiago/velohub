...
import { User as UserIcon, Building2, Lock, Save, CheckCircle, Clock, Fingerprint, Crown, Zap, AlertCircle, FileText, RotateCcw, Share2, BarChart3, Users, RefreshCw, Phone, MapPin, Loader, Trash2 } from 'lucide-react';
import { useVelohub } from '@/shared/contexts/VelohubContext'; // Importar contexto para refresh
import { fetchCepInfo } from '@/shared/lib/utils';
...
export const ProfilePage: React.FC<ProfilePageProps> = ({ user, onUpdateUser, onLogout }) => {
  ...
  const restoreDefaultContract = () => {
      setConfirmModal({
          isOpen: true,
          title: 'Restaurar contrato padrão?',
          message: 'Isso apagará seu texto personalizado e restaurará o modelo padrão do sistema. Esta ação não pode ser desfeita.',
          variant: 'warning',
  ...
  const handleDeleteAccount = () => {
      setConfirmModal({
          ...
          message: 'Esta ação é irreversível e irá excluir permanentemente sua conta, todos os veículos, vendas, despesas e dados associados. Você não poderá recuperar essas informações.',
          variant: 'danger',
          confirmText: 'Excluir Conta Permanentemente',
          onConfirm: async () => {
              setIsLoading(true);
              try {
              ...
              <div className="p-4 bg-rose-500/5 border border-rose-500/20 rounded-xl">
                  <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                          <h4 className="font-bold text-white mb-1">Excluir Conta</h4>
                          <p className="text-sm text-slate-400 mb-4">
                              Esta ação é permanente e irá excluir todos os seus dados, incluindo veículos, vendas, despesas e configurações. Não é possível desfazer esta ação.
                          </p>
                          <Button 
                              variant="ghost" 
