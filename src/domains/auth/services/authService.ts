...
export const AuthService = {
  ...
  deleteUser: async (userId: string): Promise<void> => {
      ...
      // Primeiro, exclui o perfil do usuário na tabela users
      const { error: profileError } = await supabase
          .from('users')
          .delete()
          .eq('id', userId);
      
      if (profileError) throw new Error("Erro ao excluir perfil do usuário.");
      
      // Depois, exclui a conta do Auth (se for o próprio usuário)
      const { data: { user: currentUser } } = await supabase.auth.getUser();
      if (currentUser && currentUser.id === userId) {
          const { error: authError } = await supabase.auth.admin.deleteUser(userId);
          // Note: admin.deleteUser requer privilégios de admin, 
          // então vamos usar a função de usuário para deletar a própria conta
