...
import { Vehicle, User, checkPermission, Page } from '@/shared/types';
import { formatCurrency, calculateRealProfit, maskCurrencyInput, parseCurrencyInput, parseISODate } from '@/shared/lib/utils';
...
export const Dashboard: React.FC<DashboardProps> = ({ vehicles, user }) => {
  ...
  // Attention Logic: Cars in stock > 60 days
    ...
    const profitChartData = useMemo(() => {
        ...
        const useDailyKeys = chartPeriod === 'this_month' || chartPeriod === 'last_month' || chartPeriod === 'custom';
        const includeYear = chartPeriod === 'custom' && chartStart.getFullYear() !== chartEnd.getFullYear();
        ...
        filteredChartVehicles.forEach((v) => {
            const soldAt = parseSoldDate(v.soldDate);
            if (!soldAt) return;
            const key = useDailyKeys
                ? getDayKey(soldAt, includeYear)
                : `${months[soldAt.getMonth()]}/${soldAt.getFullYear().toString().slice(2)}`;
            if (Object.prototype.hasOwnProperty.call(data, key)) {
                data[key] += calculateRealProfit(v);
            }
        });
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {canViewCosts ? (
            <StatCard 
                ...
                helpText="Soma do valor de compra + despesas + comissoes de todos os veiculos em estoque."
            />
        ) : (
            <StatCard 
        ...
        {canViewCosts ? (
            <StatCard 
                ...
                helpText="Soma do lucro real de todas as vendas: Venda - (Compra + Despesas do veiculo)."
            />
        ) : (
            <StatCard 
        ...
        {canViewCosts ? (
            <StatCard 
                ...
                helpText="Estimativa de lucro do estoque atual usando o preco de venda esperado menos o capital investido."
            />
        ) : (
            <StatCard 
        ...
        <StatCard 
          ...
          trend={attentionVehicles.length > 0 ? "down" : "neutral"}
                    ...
                    helpText="Veiculos em estoque ha mais de 60 dias. Sinal de giro lento e capital parado."
        />
      </div>
